...
export const DropdownStatus: React.FC<ListProps> = ({
  ...
  const [selectedSubStatusOptions, setSelectedSubStatusOptions] = useState<CheckboxValueType[]>([])
  ...
  const handleOptionSelect = (value: string) => {
    ...
    if (value === 'PENDING') {
      ...
      if (optionIndex === -1) {
        if (updatedSelectedOptions.includes('PENDING') && selectedOptionsSub.length < 3) {
          setSelectedOptionsSub([])
          setSelectedSubStatusOptions([])
  ...
  const handleSubStatusGroupChange = (newSelectedOptions: CheckboxValueType[]) => {
    ...
    setIndeterminatePending(selectedSubStatusCount > 0 && selectedSubStatusCount < 3)
      ...
      label: (
        <>
          <Checkbox
            ...
            value={v.value}
            checked={
              selectedOptions.includes(v.value) || (v.value === 'PENDING' && checkAllPending)
            }
            ...
            <div className='row'>
              <div className='col-lg-8' style={{ textAlign: 'start' }}>
                {selectedOptionsSub.length > 0 ? (
                  selectedOptionsSub.length > 0 && selectedOptionsSub.length < 3 ? (
                    <span style={{ color: color.font, marginLeft: 8 }}>
                      {[selectedOptionsSub.map((option) => STATUS_SEARCH[option.toString()])].join(
                        ', ',
                      )}
                    </span>
                  ) : (
                    <span style={{ color: color.font, marginLeft: 8 }}>
                      {selectedOptions.map((option) => STATUS_SEARCH[option]).length === 3
                        ? 'เลือกทั้งหมด'
                        : [selectedOptions.map((option) => STATUS_SEARCH[option])].join(', ')}
                    </span>
                  )

...
const RenderOffline: React.FC<RenderMobile> = ({ id }) => {
  ...
  useEffect(() => {
    CouponDataSource.getAllOffline(id, page, 10).then((res) => {
      setCount(res.count)
  ...
  const statusRender = (status: boolean) => {
    return status ? (
      <div className='d-flex'>
        <span className='ps-2 text-error'>
          <Image src={icon.pointDisabled} width={10} height={10} /> ถูกใช้งาน
        </span>
      </div>
    ) : (
      <div className='d-flex'>
        <span className='ps-2 text-warning'>
          <Image src={icon.pointWarning} width={10} height={10} /> ยังไม่ได้ถูกใช้งาน
        </span>
      </div>
    )
  }
        ...
        <div>
          ...
          >
            {data.map((item: any, index: number) => (
              <div
                className='py-4 px-3 d-flex flex-row justify-content-between align-items-center border-bottom'
                style={{
                  width: '100%',
                }}
              >
                ...
                <div>{statusRender(item.used)}</div>
              </div>
            ))}
            <div className='p-3'>
              <Pagination

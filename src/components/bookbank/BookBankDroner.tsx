...
const BookBankDroner: React.FC<BookBankDronerProps> = ({ callBack, data, dronerId }) => {
  const [form] = Form.useForm()
  const [bankImg, setBankImg] = useState<any>()
  const [bank, setBank] = useState<any>()
  const [bankImgCreate, setBankImgCreate] = useState<UploadImageEntity>(ImageEntity_INTI)
  ...
  const removeImg = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const getImg = data.file && data.file.filter((x) => x.category === 'BOOK_BANK')[0]
    ...
    <div className='col-lg'>
      <CardContainer>
        ...
        >
          ...
          <div className='pt-2'>
            <img src={icon.arrowDownWhite} style={{ width: '16px', height: '9px' }} />
          </div>
        </div>
          ...
          <div className='form-group pb-3'>
            ...
            <div>
              <Row
                ...
                gutter={8}
              >
                <Col span={6} className='align-self-center' onClick={onPreviewImg}>
                  <span
                    style={{
            ...
            >
              <input key={bankImg} type='file' onChange={onChangeImage} title='เลือกรูป' />
            </div>
          </div>
          <div className='form-group '>
            <label>เงื่อนไขพิเศษ</label>
            <Form.Item name='isConsentBookBank'>
              <Checkbox onChange={confirmData} defaultChecked={data?.isConsentBookBank}>

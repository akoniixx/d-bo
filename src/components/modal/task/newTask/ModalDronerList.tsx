...
const ModalDronerList: React.FC<ModalDronerListProps> = ({ show, backButton, taskId, title }) => {
  ...
  const fetchDronerList = async () => {
    await TaskDronerTempDataSource.getDronerList(taskId).then((res) => {
      res.sort((a, b) => (parseFloat(a.distance) > parseFloat(b.distance) ? 1 : -1))
      setData(res)
    })
  }
          ...
          children: (
            <>
              ...
              <span style={{ color: color.Grey }}>
                {data.droner_code}
                {data?.nickname && <ShowNickName data={data?.nickname} menu='INFO' />}
              </span>
            </>
          ),
        }
            ...
            <>
              {checkRating() ? (
                <Row>
                  ...
                  <span className='pt-2 ps-1'>{parseFloat(data.rating_avg).toFixed(2)}</span>
                </Row>
              ) : (
                <p>-</p>
              )}
    ...
    },
    {
      ...
      render: (value: any, row: any, index: number) => {
        ...
        return {
          children: (
            <>
              <Avatar size={25} src={data.logo_drone_brand} style={{ marginRight: '5px' }} />
              <span>{data.drone_brand}</span>
              {data.count_drone > 1 && <p style={{ color: color.Grey }}>(มากกว่า 1 ยี่ห้อ)</p>}
            </>
          ),
        }
      },

...
export class DroneDatasource {
  ...
  //DroneBrand
  static getDroneBrandList(search?: string): Promise<DroneBrandListEntity> {
    const params = {
      search: search,
    }
    return httpClient
  ...
  static getDrone(page: number, row: number, droneBrandId: string): Promise<any> {
    const params = {
      page: page,
      take: row,
      droneBrandId: droneBrandId,
  ...
  static addDroneBrand(data: CreateDroneBrandEntity): Promise<any> {
    const id: any = 'id'
    const droneBrandId: any = 'droneBrandId'

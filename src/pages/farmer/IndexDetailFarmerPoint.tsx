  ...
  const pageTitle = (
    <div className='container d-flex justify-content-between' style={{ padding: '10px' }}>
      <div className='col-lg-4'>
        <Search placeholder='ค้นหาชื่อที่มาของคะแนน' className='col-lg-12 p-1' />
      </div>
      <div className='col-lg-3'>
        <Select className='col-lg-12 p-1' placeholder='ได้รับ/แลกคะแนน' allowClear></Select>
      </div>
      ...
      render: (value: any, row: any, index: number) => {
        return {
          children: (
            <span
              style={{
                color: row.pointType === 'ได้รับคะแนน' ? '#A9CB62' : color.Error,
              }}
            >
              {row.pointType}
            </span>
            ...
            <div className='d-flex flex-row justify-content-center'>
              <div className='col-lg-6'>
                <ActionButton
                  ...
                  onClick={() => navigate('/DetailFarmerPoint/id=' + (index + 1))}
                />
              </div>
            </div>
          ),
    ...
    <>
      ...
      <CardContainer>
        <Table
          ...
          size='large'
          tableLayout='fixed'
          rowClassName={(a) => (a.pointType == 'แลกคะแนน' ? 'table-row-older' : 'table-row-lasted')}
        />
      </CardContainer>
...
export default IndexDetailFarmerPoint

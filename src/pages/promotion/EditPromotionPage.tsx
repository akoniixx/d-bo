...
function EditPromotion(){
  ...
  const [form] = Form.useForm();
    ...
    const getPromotion = (id : string)=>{
      ...
      .then(async res=>{
        form.setFieldsValue({
          ...
          DateStart : moment(new Date(res.startDate).toUTCString()),
          TimeStart : moment(new Date(res.startDate).getTime()),
          DateExpired : moment(new Date(res.expiredDate).toUTCString()),
          TimeExpired : moment(new Date(res.expiredDate).getTime()),
        ...
        setRenderMobile(
          ...
          promotionStatus : res.promotionStatus,
          count : res.count,
          startDate : changeFormat(moment(res.startDate).format("YYYY-MM-DD")),
          expiredDate : changeFormat(moment(res.expiredDate).format("YYYY-MM-DD")),
          startTime : moment(res.startDate).format("HH:mm"),
          expiredTime : moment(res.expiredDate).format("HH:mm"),
          expiredDateTitle : "",
    ...
    const warningTime = () => {
      ...
      const startDate = new Date(moment(DateStart).format("YYYY-MM-DD")+" "+moment(TimeStart).format("HH:mm:ss")).getTime()
    ...
    const onSubmit = (status? : string) => {
      ...
      const startDate = moment(DateStart).format("YYYY-MM-DD")+" "+moment(TimeStart).format("HH:mm:ss")
      const expiredDate = moment(DateExpired).format("YYYY-MM-DD")+" "+moment(TimeExpired).format("HH:mm:ss")
    ...
    return <Layouts>
        ...
        >
          ...
          <div className="d-flex justify-content-between px-4 pb-4">
            ...
            <Button style={{
              ...
              color : color.White
            }} onClick={()=>warningTime()}>ยืนยัน</Button>
          </div>
        </Modal>
        <Modal
          title="คำเตือน"
        ...
        >
          <div className="px-4 pt-4">
            <span className="text-secondary">เนื่องจากวันเริ่มใช้งานของคูปองนั้นเกิดขึ้นในอนาคต ระบบจะบันทึกคูปองให้อยู่ในสถานะแบบร่างก่อนเพื่อป้องกันการใช้งานก่อนกำหนดและจะเปลี่ยนสถานะเป็นใช้งานในวันที่ท่านได้กำหนดเอาไว้</span>
          </div>
          <Divider style={{
            marginBottom : '20px'
          }}/>
                      ...
                      <div style={{
                        ...
                        <div style={{
                          ...
                          <div style={{
                            ...
                            <div>
                              <>{()=>{
                                console.log(`renderMobile.couponName = ${renderMobile.couponName}`)
                              }}</>
                              <p style={{
                                margin : '0px',
                                padding : '0px',

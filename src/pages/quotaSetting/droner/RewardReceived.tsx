...
function RewardReceived() {
    ...
    const getRewardRound = async () => {
      await CampaignDatasource.getCampaignQuota('DRONER').then((res) => {
        if (res.data) {
          ...
          const dataFilter = res.data.filter((x: any) => x.id === camId)
          setRewardRound(dataFilter.map((x: any) => x.condition[0].rewardRound)[0])
        }
      })
    }
    getRewardRound()
  ...
  useEffect(() => {
    const getQuotaReport = async () => {
      ...
      await QuotaDatasource.getAllQuotaReport(camId).then((res) => {
        if (res.data) {
          const dataFilter = res.data.filter((x) => x.dronerId === queryString[1])
          setDronerName(dataFilter.map((x) => x.firstname + ' ' + x.lastname)[0])
        }
      })
    }
    getQuotaReport()
  ...
  const getQuotaRedeem = async () => {
    const camId: any = localStorage.getItem('id')
    await QuotaDatasource.getQuotaRedeemHisId(queryString[1], camId, current, row).then((res) => {
      setData(res)
    })
  }
  ...
  const PageTitle = (
    <>
      <div className='container d-flex justify-content-start' style={{ padding: '4px' }}>

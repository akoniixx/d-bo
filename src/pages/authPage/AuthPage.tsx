...
import { MaintenanceSystem, MaintenanceSystem_INIT } from '../../entities/MaintenanceSystemEntities'
...
const AuthPage: React.FC = () => {
  ...
  const checkMaintance = () => {
    MaintenanceDataSource.getMaintenceSystem('BO').then((res) => {
      setCheckTime(
        checkTimeMaintance(moment(res.responseData.dateStart), moment(res.responseData.dateEnd)),
      )
        ...
        <div className='col-lg-6'>
          ...
          >
            ...
            <div className='text-center justify-content-center'>
              {!checkTime ? <FromLogin /> : <FromMaintenance data={dataMaintance} />}
            </div>
          </div>
        </div>
      </div>

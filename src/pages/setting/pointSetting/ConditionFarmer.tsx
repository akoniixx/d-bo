...
function ConditionFarmer() {
  ...
  const [taskId, setTaskId] = useState<PointSettingEntities>(PointSettingEntities_INIT)
  const [productId, setProductId] = useState<PointSettingEntities>(PointSettingEntities_INIT)
  ...
  const getDataPoint = async () => {
    await PointSettingDatasource.getAllPointSetting().then((res) => {
      if (res.length != 0) {
        const filterTask = res.find((x: any) => x.pointType === 'DISCOUNT_TASK')
        const filterProduct = res.find((x: any) => x.pointType === 'DISCOUNT_PRODUCT')
  ...
  const onClickSave = async () => {
    ...
    const pushAmount = Map(pushReceiveType.toJS()).set('amounts', taskId.amounts)
    ...
    const pushAmountFer = Map(pushReceiveTypeFer.toJS()).set('amounts', productId.amounts)
    await PointSettingDatasource.editPointSetting(pushAmountFer.toJS()).then((res) => {
      Swal.fire({
        title: 'บันทึกสำเร็จ',
        icon: 'success',
        timer: 1500,
  ...
  const onClickCreate = async () => {
    if (chooseDisTask === true && chooseDisFer === false) {
      ...
      await PointSettingDatasource.createPointSetting(dataPointFer).then((res) => {
        Swal.fire({
          title: 'บันทึกสำเร็จ',
          icon: 'success',
          timer: 1500,
  ...
  const renderConditionFarmer = (
    <CardContainer>
      ...
      <Form style={{ padding: '32px' }} form={form}>
        <label style={{ fontSize: '16px', paddingBottom: 10 }}>การแลก ICK Point</label>
        <br />
        <Checkbox

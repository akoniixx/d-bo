...
import { DRONER_DRONE_MAPPING, DRONE_STATUS, STATUS_COLOR } from '../../definitions/DronerStatus'
    ...
    <>
      ...
      <div className='d-flex justify-content-between pt-3'>
        ...
        <div className='col p-1'>
          ...
          >
            {seriesDrone?.map((item: any) => <option value={item.id}>{item.series}</option>)}
          </Select>
        </div>
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value: any, row: any, index: number) => {
        ...
        const nameDrone = droneBrand?.filter((x) => x.id == filterDrone)[0]
        return {
          children: (
            <div className='container'>
              <span className='text-dark-75  d-block font-size-lg'>
                {nameDrone?.logoImagePath ? (
                  <Avatar size={25} src={nameDrone.logoImagePath} style={{ marginRight: '5px' }} />
                ) : (
                  <Avatar size={25} style={{ color: '#0068F4', backgroundColor: '#EFF2F9' }}>
            ...
            <>
              <Row>
                {getLicenseDrone != false ? (
                  ...
                  >
                    <FileTextFilled />
                    <text onClick={previewLicenseDrone}>ใบอนุญาตโดรน(กสทช.)</text>
                  </div>
                ) : (
                  '-'
                )}
          ...
          children: (
            <>
              <span style={{ color: STATUS_COLOR[row.status] }}>
                <Badge color={STATUS_COLOR[row.status]} /> {DRONER_DRONE_MAPPING[row.status]}
                <br />
              </span>

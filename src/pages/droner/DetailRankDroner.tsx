...
function DetailRankDroner() {
  ...
  const fetchDronerById = async () => {
    setLoading(true)
    await DronerRankDatasource.getDronerRankById(dronerId, current, row, sortField, sortDirection)
      .then((res) => {
        setData(res)
        ...
        <div className='row'>
          <div className='form-group text-center pb-4'>
            <div
              ...
              style={{
                cursor: 'pointer',
                backgroundImage: `url(${imgProfile == undefined ? image.empty_img : imgProfile})`,
              }}
            />
          </div>
        </div>
        <div className='row text-center pb-2'>
          <CardContainer style={style}>
            ...
            <span style={{ color: color.Success }}>
              {data.totalTaskCount ? formatNumberWithCommas(parseFloat(data.totalTaskCount)) : 0}{' '}
              งาน
            </span>
          </CardContainer>
          <CardContainer style={style}>
            ...
            <br />
            <span style={{ color: color.Success }}>
              {data.totalRaiCount ? formatNumberWithCommas(parseFloat(data.totalRaiCount)) : 0} ไร่
            </span>
          </CardContainer>
          <CardContainer style={style}>
        ...
        <div className='row'>
          ...
          <div className='row'>
            <Form.Item name='droneId'>
              <Input disabled defaultValue={data.dronerCode !== null ? data.dronerCode : '-'} />
            </Form.Item>
          </div>
          <div className='row'>
            <div className='col-lg-6'>
              ...
              <Form.Item name='firstname'>
                <Input disabled defaultValue={data.firstname ? data.firstname : '-'} />
              </Form.Item>
            </div>
            <div className='col-lg-6'>
              <label>นามสกุล</label>
              <Form.Item name='lastname'>
                <Input disabled defaultValue={data.lastname ? data.lastname : '-'} />
              </Form.Item>
            </div>
          </div>
          <div className='row'>
            <div className='col-lg-6'>
              ...
              <Form.Item name='telephoneNo'>
                <Input disabled defaultValue={data.telephoneNo ? data.telephoneNo : '-'} />
              </Form.Item>
            </div>
            <div className='col-lg-6'>
              <label>ชื่อเล่น</label>
              <Form.Item name='nickname'>
                <Input disabled defaultValue={data.nickname !== null ? data.nickname : '-'} />
              </Form.Item>
            </div>
          </div>
          <div className='row'>
            <div className='col-lg-6'>
              ...
              <Form.Item name='subdistrictName'>
                <Input
                  disabled
                  defaultValue={
                    data.address !== null ? data.address.subdistrict.subdistrictName : '-'
                  }
                />
              </Form.Item>
            </div>
            <div className='col-lg-6'>
              ...
              <Form.Item name='districtName'>
                <Input
                  disabled
                  defaultValue={data.address !== null ? data.address.district.districtName : '-'}
                />
              </Form.Item>
            </div>
            <div className='col-lg-6'>
              ...
              <Form.Item name='provinceName'>
                <Input
                  disabled
                  defaultValue={data.address != null ? data.address.province.provinceName : '-'}
                />
              </Form.Item>
            </div>
          </div>
    ...
    },
    {
      ...
      render: (value: any, row: any, index: number) => {
        return {
          children: (
            <>
              <span>{parseFloat(row.farmAreaAmount).toFixed(1) + ' ' + 'ไร่'}</span>
            </>
          ),
        }
      },
      ...
      render: (value: any, row: any, index: number) => {
        return {
          children: (
            <>
              <span>
                {row.farmer.address.provinceId ? row.farmer.address.province.provinceName : '-'}
              </span>
            </>
          ),
        }
              ...
              <span>
                {row.reviewDronerAvg > '0' ? (
                  <Row>
                    ...
                    <span className='pt-1 ps-1'>{parseFloat(row.reviewDronerAvg).toFixed(1)}</span>
                  </Row>
                ) : (
                  '-'
                )}
      ...
      <Spin tip='กำลังโหลดข้อมูล...' size='large' spinning={loading}>
        ...
        <div className='row '>
          ...
          <div className='col-lg-8'>
            ...
            <div className='d-flex justify-content-between col-lg' style={{ margin: '10px' }}>

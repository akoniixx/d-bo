...
const EditDronerMission = () => {
            ...
            <div className='d-flex flex-row justify-content-center'>
              <div className='col-lg-4'>
                <ActionButton
                  ...
                  actionDisable={
                    form.getFieldValue('status') !== 'ACTIVE' && count > 1 ? row.isDelete : true
                  }
  ...
  const onFieldsChange = () => {
    const { missionName, campaignType, startDate, endDate, status } = form.getFieldsValue()
  ...
  const submit = async () => {
    ...
    const condition = dataSub?.map((y: any, i: number) => {
      ...
      return {
        num: i + 1,
        missionName: fs[`${y.num}_missionName`],
        rai: mapNum ? parseFloat(fs[`${y.num}_rai`]).toFixed(2) : parseFloat(fs[`${y.num}_rai`]),
        rewardId: fs[`${y.num}_rewardId`],
        point: fs[`${y.num}_point`],
        descriptionReward: fs[`${y.num}_description`],
        conditionReward: fs[`${y.num}_condition`],

...
function PinMain({ currentApp, form }: Props) {
  const listDnd: ListDndType[] = form.getFieldValue('pinMain')
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const _ = useWatch('pinMain', form) // for re-render
  ...
  const onSelectNews = async (
    ...
    const newList = [...listDnd]
    const deletePinMain = form.getFieldValue('deletePinMain') || []
    const filterDeletePinMain = deletePinMain.filter((el: { newsId: string }) => {
    ...
    form.setFieldsValue({
      pinMain: newList,
      deletePinMain: filterDeletePinMain,
    })
  ...
  const onDelPinNews = async (v: any) => {
    const newList = [...listDnd]
    const deletePinMain = form.getFieldValue('deletePinMain') || []
    const alreadyDeleted = deletePinMain.find(
      (item: { newsId: string }) => item.newsId === v.newsId,
    )

...
const AddNewTask = () => {
  ...
  const handleSelectFarmerPlot = (value: any) => {
    ...
    form.setFieldsValue({
      farmAreaAmount: plotSelected?.raiAmount,
    });
                ...
                <div className="form-group col-lg-4">
                  ...
                  <Form.Item
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        setCreateNewTask((prev) => ({
                          ...prev,
                          farmAreaAmount: e.target.value,
                        }));
                      }}
                      disabled={!farmerPlotSeleced} // แก้ไขจำนวนไร่
                    />
                  </Form.Item>
                </div>
              </div>
            ...
            <>
              ...
              {row.rating_avg !== null && (
                <Tooltip title={tooltipTitle} className="p-2">
                  <img src={icon.iconReviewDroner} alt="ic_review" />
                </Tooltip>
              )}
              <br />
  ...
  const checkValidateStep = (
    ...
    if (currentStep === 0) {
      ...
      let checkEmptyNumber = ![data.price, data.unitPrice].includes(
        0
      );
  ...
  const nextStep = () => {
    if (current === 0) {
      ...
      const a = Map(createNewTask).set(
        "taskDronerTemp",
        pushDronerList
      );
      if (selectionType === "checkbox") {
        const s = Map(a.toJS()).set("status", "WAIT_RECEIVE");
        const f = Map(s.toJS()).set(
          "fee",
        ...
        const d = Map(s.toJS()).set(
          "dronerId",
          dronerSelected[0].droner_id
        );

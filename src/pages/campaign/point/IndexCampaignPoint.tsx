...
const IndexCampaignPoint = () => {
  ...
  const pageTitle = (
    <>
      <Row className='container d-flex justify-content-between' style={{ padding: '10px' }}>
        <Col span={14}>
          <span
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value: any, row: any, index: number) => {
        return {
          children: <span>{'+' + row.condition[0].point + ' แต้ม / ไร่'}</span>,
        }
      },
    },
    {
      ...
      render: (value: any, row: any, index: number) => {
        return {
          children: (
            ...
            >
              <Badge color={STATUS_COLOR[row.status]} /> {STATUS_COUPON[row.status]}
            </span>
          ),
        }
      },

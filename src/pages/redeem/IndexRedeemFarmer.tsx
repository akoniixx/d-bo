...
const IndexRedeemFarmer = () => {
  const navigate = useNavigate()
  const dateFormat = 'DD/MM/YYYY'
  const [current, setCurrent] = useState(1)
  const [dataFarmer, setDataFarmer] = useState<RedeemDronerListEntity>()
  ...
  const fetchRedeemFarmer = () => {
    setLoading(true)
    RedeemDatasource.getFarmerTransactions(
      5,
      current,
      searchKeyword,
      searchStartDate,
    ...
    <>
      <Row justify={'space-between'} style={{ padding: '10px' }}>
        <Col span={10}>
          ...
          >
            <strong>ประวัติการแลก (เกษตรกร)</strong>
          </span>
        </Col>
  ...
  const columnsFarmer = [
    ...
    },
    {
      ...
      render: (value: any, row: any, index: number) => {
        return {
          children: <span>{row.receiverDetail ? row.receiverDetail.tel : '-'}</span>,
        }
      },
    },
    {
  ...
  const tableDronerFix = columnsFarmer.map((item, idx) => {
    if (idx < 2) {
      return {
        ...item,
        fixed: true,
      ...
      >
        <Row>
          ...
          <Col span={10} className='p-2'>
            <span style={{ fontWeight: 'bold' }}>ชื่อของรางวัล</span>
  ...
  return (
    <>
      ...
      <div className='d-flex justify-content-between pt-3 pb-3'>
        <p>รายการทั้งหมด {dataFarmer?.count} รายการ</p>
        <Pagination
          current={current}
          total={dataFarmer?.count}
...
export default IndexRedeemFarmer

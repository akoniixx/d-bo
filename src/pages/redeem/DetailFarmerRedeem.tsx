...
const DetailFarmerRedeem = () => {
  ...
  const [dataUpdate, setDataUpdate] = useState<UpdateRedeemFarmerEntity>(
    UpdateRedeemFarmerEntity_INIT,
  )
  const [delivery, setDelivery] = useState<DeliveryListEntity>()
  const [exchange, setExchange] = useState('')
  const [data, setData] = useState<DetailRedeemFarmerEntity>()
  ...
  const fetchDetail = () => {
    RedeemDatasource.getRedeemFarmerTransactionsById(queryString[1]).then((res) => {
  ...
  const submit = async () => {
    ...
    update.farmerTransactionId = data?.id
    ...
    RedeemDatasource.updateStatusFarmerRedeem(update).then((res) => {
      if (res.id) {
        Swal.fire({
          title: 'บันทึกสำเร็จ',
          icon: 'success',
  ...
  const renderDronerDetail = (
    <CardContainer>
      <CardHeader textHeader='ข้อมูลเกษตรกร' bgColor='#219653' />
      ...
      <NewTable
        columns={
          data?.redeemDetail.rewardType === 'PHYSICAL' ? columnHisPhysical : columnHisDigital
        }
        dataSource={data?.farmerRedeemHistories}
        pagination={false}
      />
    </CardContainer>
  )
